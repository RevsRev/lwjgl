...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    vec3 ambient = light.ambient * material.ambient * vec3(texture(aTexture,
                                                TexCoords));
    vec3 diffuse = light.diffuse * material.diffuse * diff * vec3(texture(aTexture,
                                                       TexCoords));
    vec3 specular = light.specular * material.specular * spec * vec3(texture(aTexture,
                                                         TexCoords));
